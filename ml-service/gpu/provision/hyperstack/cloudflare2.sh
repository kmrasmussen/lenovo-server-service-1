...
# Setup logging
SETUP_LOG="${SCRIPT_DIR}/setuplog.txt"
exec > >(tee -a "$SETUP_LOG")
exec 2>&1

echo "=== Starting Combined GPU Worker Setup ===" >> "$SETUP_LOG"
echo "$(date): Beginning setup process" >> "$SETUP_LOG"
echo "Script directory: $SCRIPT_DIR" >> "$SETUP_LOG"
echo "Script user: $SCRIPT_USER" >> "$SETUP_LOG"

# --- CONFIGURATION ---
CF_API_TOKEN="qzTtVSSeVHeJHSHAuk0ZO2-WQOGIUcJRu6vZhIWQ"
CF_ZONE_ID="7d7e4955a596f824e24c3205a9c711c9"
...
create_tunnel() {
    ...
    # Get account ID
    ACCOUNT_RESPONSE=$(curl -s -X GET "https://api.cloudflare.com/client/v4/accounts" -H "Authorization: Bearer qzTtVSSeVHeJHSHAuk0ZO2-WQOGIUcJRu6vZhIWQ")
    ...
    TUNNELS_RESPONSE=$(curl -s -X GET "https://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/cfd_tunnel" -H "Authorization: Bearer qzTtVSSeVHeJHSHAuk0ZO2-WQOGIUcJRu6vZhIWQ")
    ...
    if [ ! -z "$EXISTING_TUNNEL" ]; then
        ...
        if [ ! -f "/etc/cloudflared/${TUNNEL_ID}.json" ]; then
            ...
            curl -s -X DELETE "https://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/cfd_tunnel/${TUNNEL_ID}" \
                -H "Authorization: Bearer qzTtVSSeVHeJHSHAuk0ZO2-WQOGIUcJRu6vZhIWQ" > /dev/null
            ...
            TUNNEL_RESPONSE=$(curl -s -X POST "https://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/cfd_tunnel" \
                -H "Authorization: Bearer qzTtVSSeVHeJHSHAuk0ZO2-WQOGIUcJRu6vZhIWQ" \
...
setup_dns() {
    ...
    for hostname in "${hostnames[@]}"; do
        ...
        API_RESPONSE=$(curl -s -X POST "https://api.cloudflare.com/client/v4/zones/7d7e4955a596f824e24c3205a9c711c9/dns_records" \
             -H "Authorization: Bearer qzTtVSSeVHeJHSHAuk0ZO2-WQOGIUcJRu6vZhIWQ" \
             -H "Content-Type: application/json" \
...
setup_voiceagent_server() {
  ...
  echo "Changing to script directory: $SCRIPT_DIR" >> "$SETUP_LOG"
  cd "$SCRIPT_DIR"
...
Type=simple
User=$SCRIPT_USER
WorkingDirectory=$SCRIPT_DIR/kyutai-voiceagent-rust
ExecStart=$SCRIPT_DIR/kyutai-voiceagent-rust/target/release/kyutai-voiceagent-rust
Restart=on-failure
RestartSec=10s
Environment=HOME=$SCRIPT_DIR
Environment=PATH=$SCRIPT_DIR/.cargo/bin:/usr/local/bin:/usr/bin:/bin
StandardOutput=append:$SCRIPT_DIR/voiceagent_log.txt
StandardError=append:$SCRIPT_DIR/voiceagent_log.txt
...
EOF
    ...
    echo "Voice agent logs will be written to $SCRIPT_DIR/voiceagent_log.txt" >> "$SETUP_LOG"
    echo "" >> "$SETUP_LOG"
}
...
setup_moshi_server() {
    ...
    mkdir -p "$SCRIPT_DIR/ran_downloadandrundotsh"
...
Type=simple
User=$SCRIPT_USER
WorkingDirectory=$SCRIPT_DIR/delayed-streams-modeling
ExecStart=$SCRIPT_DIR/delayed-streams-modeling/moshi-server worker --config configs/config-stt-en_fr-hf.toml
Restart=on-failure
RestartSec=10s
Environment=HOME=$SCRIPT_DIR
StandardOutput=append:$SCRIPT_DIR/moshilog.txt
StandardError=append:$SCRIPT_DIR/moshilog.txt
...
EOF
    ...
    echo "Moshi server logs will be written to $SCRIPT_DIR/moshilog.txt" >> "$SETUP_LOG"
    echo "" >> "$SETUP_LOG"
}
...
main() {
    ...
    echo "  tail -f $SCRIPT_DIR/moshilog.txt  # Moshi server logs" >> "$SETUP_LOG"
    echo "  tail -f $SCRIPT_DIR/voiceagent_log.txt  # Voice agent logs" >> "$SETUP_LOG"
